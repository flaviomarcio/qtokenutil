...
#define dPvt() auto &p = *reinterpret_cast<TokenUtilPvt *>(this->p)
...
TokenUtil::TokenUtil(const QByteArray &secret,
                     const QHash<QByteArray, QByteArray> &payload,
                     QObject *parent)
    : QObject{parent}
{
    this->p = new TokenUtilPvt();
...
Token TokenUtil::readFromToken(const QByteArray &token)
{
    ...
    __returnToken.algorithm = TokenUtil::algorithmType().value(
        m_jwtObj.getAlgorithmStr().toLower().toUtf8());
    __returnToken.secret.clear();
    __returnToken.tokenPayLoad = m_jwtObj.getPayloadJDoc().object().toVariantHash();
    __returnToken.tokenScope = __returnToken.tokenPayLoad.value(QByteArrayLiteral("scope"))
                                   .toByteArray();
    __returnToken.tokenIat.setSecsSinceEpoch(
        __returnToken.tokenPayLoad.value(QByteArrayLiteral("iat")).toLongLong());
    __returnToken.tokenExp.setSecsSinceEpoch(
        __returnToken.tokenPayLoad.value(QByteArrayLiteral("exp")).toLongLong());
...
Token TokenUtil::generateToken(const QByteArray &secret,
                               const QHash<QByteArray, QByteArray> &payload,
                               const eTokAlgorithm &algorithm,
                               const QDateTime &expires_in)
{
